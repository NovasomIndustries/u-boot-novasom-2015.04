...
CONFIG_BOOTCOMMAND="mmc dev ${mmcdev}; if mmc rescan; then if run mmcloadbootscript; then run bootscript; else if test -n ${bootenv}; then if run mmcloadbootenv; then echo Loaded environment ${bootenv};run importbootenv;if test -n ${uenvcmd}; then run uenvcmd;else echo uenvcmd not found;fi;fi;fi;if run mmcloadimage; then if run mmcloadfdt; then if run mmcloadinitrd; then run board_boot; fi; fi; fi; fi; fi; usb start;if run usbloadbootscript; then run bootscript; else if run usbloadimage; then if run usbloadfdt; then if run usbloadinitrd; then run board_boot; fi; fi; fi; fi; fi; run netboot;"
...
CONFIG_DEFAULT_FDT_FILE="novasoms.dtb"
...
CONFIG_BOARDDIR="board/novasoms"
...
CONFIG_EXTRA_ENV_SETTINGS="setenv splashimage_mmc_dev 00setenv splashimage_mmc_part 10bootenv=NOVAsomParams0splashimage=0x108000000setenv splashpos m,m0splashimage_file_name=splash.bmp.gz0script=boot.scr0image=zImage0initrd=uInitrd0fdt_file=" CONFIG_DEFAULT_FDT_FILE "0console=ttymxc00splashpos=m,m0fdt_high=0xffffffff0initrd_high=0xffffffff0fdt_addr=0x180000000fsaddr=0x128000000boot_fdt=try0ip_dyn=yes0ethaddr=5c:b8:b2:91:9f:290mmcdev=" __stringify(CONFIG_SYS_MMC_ENV_DEV) "0mmcpart=10usbdev=00usbpart=10mmcroot=/dev/mmcblk2p2 rootwait rw0ramroot=/dev/ram rootwait rw0update_sd_firmware_filename=u-boot.imx0update_sd_firmware=if test ${ip_dyn} = yes; then setenv get_cmd dhcp; else setenv get_cmd tftp; fi; if mmc dev ${mmcdev}; then if ${get_cmd} ${update_sd_firmware_filename}; then setexpr fw_sz ${filesize} / 0x200; setexpr fw_sz ${fw_sz} + 1; mmc write ${loadaddr} 0x2 ${fw_sz}; fi; fi0bootscript=echo Running bootscript from mmc ...; source0mmcloadbootscript=fatload mmc ${mmcdev}:${mmcpart} ${loadaddr} ${script};0mmcloadimage=fatload mmc ${mmcdev}:${mmcpart} ${loadaddr} ${image}0mmcloadinitrd=fatload mmc ${mmcdev}:${mmcpart} ${fsaddr} ${initrd}0mmcloadfdt=fatload mmc ${mmcdev}:${mmcpart} ${fdt_addr} ${fdt_file}0mmcloadbootenv=fatload mmc ${mmcdev}:${mmcpart}  ${loadaddr} ${bootenv}0importbootenv=echo Importing environment ...; env import -t ${loadaddr} ${filesize}0usbloadbootscript=fatload usb ${usbdev}:${usbpart} ${loadaddr} ${script};0usbloadimage=fatload usb ${usbdev}:${usbpart} ${loadaddr} ${image}0usbloadinitrd=fatload usb ${usbdev}:${usbpart} ${fsaddr} ${initrd}0usbloadfdt=fatload usb ${usbdev}:${usbpart} ${fdt_addr} ${fdtfile}0boardargs=setenv bootargs console=${console},${baudrate} root=${ramroot} video=mxcfb0:dev=hdmi,1280x720M@60,if=RGB24 fbmem=28M ramdisk_size=96000;0board_boot=echo Booting ...; run boardargs; bootz ${loadaddr} ${fsaddr} ${fdt_addr};0netargs=setenv bootargs console=${console},${baudrate} root=/dev/nfs ip=dhcp nfsroot=${serverip}:${nfsroot},v3,tcp0netboot=echo Booting from net ...; run netargs; if test ${ip_dyn} = yes; then setenv get_cmd dhcp; else setenv get_cmd tftp; fi; ${get_cmd} ${image}; if test ${boot_fdt} = yes || test ${boot_fdt} = try; then if ${get_cmd} ${fdt_addr} ${fdtfile}; then bootz ${loadaddr} - ${fdt_addr}; else if test ${boot_fdt} = try; then bootz; else echo WARN: Cannot load the DT; fi; fi; else bootz; fi;0"
CONFIG_GPT_TIMER=y
...
CONFIG_MACH_TYPE="MACH_TYPE_NOVASOMS"
